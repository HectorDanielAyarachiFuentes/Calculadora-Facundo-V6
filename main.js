...
export async function reExecuteOperationFromHistory(historyInput) {
  ...
  try {
    ...
    ErrorHandler.mostrarError(salida, 'invalidOperation', { error });
...
async function calcular(addToHistory = true) {
  ...
  if (!ErrorHandler.validarOperacion(salida, entrada)) {
    bajarteclado();
    actualizarEstadoDivisionUI(false);
    return;
  }
  ...
  try {
    switch (operador) {
      ...
      case '/':
        ...
        break;
      default:
        ErrorHandler.mostrarError(salida, 'invalidOperation');
    }
  } catch (error) {
